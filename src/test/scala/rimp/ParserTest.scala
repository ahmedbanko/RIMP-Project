...
class ParserTest extends AnyFunSuite with BeforeAndAfterAll with BeforeAndAfterEach {
  ...
  test("Test translateRev function") {
    assert(p.translateRev(fixtures.EX1) == ex1_inverted)
    p.resetCounters
    assert(p.translateRev(fixtures.EX2) == ex2_inverted)
    p.resetCounters
    assert(p.translateRev(fixtures.EX3) == ex3_inverted)
    p.resetCounters
    assert(p.translateRev(fixtures.EX4) == ex4_inverted)
    p.resetCounters
    assert(p.translateRev(fixtures.EX5) == ex5_inverted)
    p.resetCounters
    assert(p.translateRev(fixtures.arrProg1) == arrProg1_inverted)
    p.resetCounters
    assert(p.translateRev(fixtures.reverse_arr_prog) == reverse_arr_prog_inverted)
  }
