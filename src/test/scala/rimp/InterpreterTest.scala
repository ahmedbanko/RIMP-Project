...
class InterpreterTest extends AnyFunSuite with BeforeAndAfterAll with BeforeAndAfterEach {
  ...
  test("Test evaluation of arithmatic expressions") {
    ...
    env = env + ("x" -> (10, interp.Assign("x", interp.Num(10))))
...
//    assert(interp.eval_stmt(interp.While(interp.False, List(interp.Skip), 0), env) == env)
//    env = interp.eval_stmt(interp.While(interp.Bop(">", interp.Var("i"), interp.Num(0)), List(interp.Assign("i", interp.Num(0))),0), env)
